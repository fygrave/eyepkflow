...
var sdc = new StatsD({host: 'localhost', port: 8125});
...
.on("packet",function(packet) { console.log("packet"); 
	packet.v5Flows.forEach(function(item) {
		...
		item.srcaddr.forEach(function(v) {
			g = geo.lookup(v);
			if (g != null) {
		...
		item.dstaddr.forEach(function(v) {
			g = geo.lookup(v);
			if (g != null) {
			sdc.increment('dst.country.' + g.country, item.dPkts);
			sdc.increment('dst.region.' + g.region, item.dPkts);
			sdc.increment('dst.city.' + g.city, item.dPkts);
			sdc.increment('dstbytes.country.' + g.country, item.dOctets);
			sdc.increment('dstbytes.region.' + g.region, item.dOctets);
			sdc.increment('dstbytes.city.' + g.city, item.dOctets);
			}
		});
